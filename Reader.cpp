...
Reader::Reader(std::string fname, size_t buffer_size)
{
    std::shared_ptr<std::ifstream> stream(
        new std::ifstream(fname, std::ios_base::in | std::ios_base::binary));
    if (ends_with(fname, ".mae")) {
        ...
        m_gzip_stream.reset(new boost::iostreams::filtering_streambuf<
                            boost::iostreams::input>());
        m_gzip_stream->push(boost::iostreams::gzip_decompressor());
        m_gzip_stream->push(*stream);
        std::shared_ptr<std::istream> decompressed_stream(
            new std::istream(m_gzip_stream.get()));
